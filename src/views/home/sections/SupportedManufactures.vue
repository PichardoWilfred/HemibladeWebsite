<template>
    <div class="relative flex flex-col pt-6 pb-16 border-b border-b-gray-2 min-h-[280px]">
        ...
        <section class="flex relative overflow-hidden justify-center bg-white max-lg:flex-wrap lg:h-[380px] xl:h-[320px]">

            <div class="physical-slide" :style="physical_slide">
                <div class="image-container">
                    <img :src="slide.current.img" :key="index.current" :alt="slide.current.alt" class="manufactures" :class="slide.current.class_" />
                </div>
                <p v-html="slide.current.text" class="description-text"></p>
            </div>

            <div class="visual-slide" :style="visual_slide">
                <div v-for="({ class_, text, alt, img }, index) in slide"  
                class="flex max-lg:flex-wrap justify-center bg-white h-full w-screen lg:w-full">
                    <div class="image-container">
                        <img :src="img" :key="index.current" :alt="alt" class="manufactures" :class="class_" />
                    </div>
        ...
        class="absolute hidden items-center justify-center top-0 left-0 w-[40px] h-[40px] rounded-full text-white bg-yellow-3 p-4">O</button>
...
<script setup>
    ...
    const supported_manufactures = reactive([
            {
                ...
                img: DMP,
                class_: 'w-[80%] max-lg:max-w-[180px] lg:w-[280px]',
                alt: 'DMP'
            },
            {
    ...
    const move = (direction) => {
        ...
        const slide_to = {
            forward: () => {
                visual_slide.transform = 'translateX(-33.33333%)';
            },
            backward: () => {
                visual_slide.transform = 'translateX(33.33333%)';
            }
...
<style scoped>
.image-container {
    @apply flex items-center content-center justify-center lg:ml-[6.1vw] lg:mr-[8vw];
}
img.manufactures {
    max-width: unset;
...
.physical-slide {
    @apply flex max-lg:flex-wrap relative justify-center bg-white w-full;
}
</style>